		...
		if ($res != false){
			 foreach ($res as $row){
				 if(trim($row['estusr'])=='Activo'){
					$images = "<a href='#' title='Editar Datos'  class='edi'  data-ide='".$row['nriper']."' 
					           data-nom='".$row['nomtrc']."' data-eml='".$row['emltrc']."' data-est='".trim($row['estusr'])."'
							   data-sed='".$row['codsed']."'>
							   <img src='/res/icons/sirco/edi.png' width='16'
							   height='16' /></a >&nbsp;&nbsp;&nbsp;
							   <a href='#' title='Editar Perfil'  class='per'  data-ide='".$row['nriper']."' 
							   data-nom='".$row['nomtrc']."' data-sed='".$row['codsed']."'>
							   <img src='/res/icons/sirco/seleccionar.png' width='16' height='16' /></a >";
				 }else{
					$images = " <a href='#' title='Activar Usuario'  class='upd'  data-ide='".$row['nriper']."' 
